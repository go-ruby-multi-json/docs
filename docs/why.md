...
`go-ruby-multi-json/multi-json` reimplements Ruby's `multi-json` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-multi-json/multi-json` directly, with no Ruby runtime;
...
Because `multi-json` is pure Go with cgo disabled, it also compiles to **WebAssembly**
